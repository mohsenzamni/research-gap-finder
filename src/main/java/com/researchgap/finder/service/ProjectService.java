...
public class ProjectService {
 ...
 public ProjectService(ProjectRepository p, PaperRepository pa, GapRepository g, IdeaRepository i,
                       AnalysisBoundary a, LiteratureProvider l) {
  projects = p;
  papers = pa;
  gaps = g;
  ideas = i;
 ...
 @Transactional
 public List<ResearchGap> analyze(Long id) {
  Project p = get(id);
  List<Paper> ps = papers.findByProjectId(id);
  if (ps.isEmpty()) {
   throw new IllegalStateException("Add at least one paper before analysis");
  }
  Set<String> topics = new LinkedHashSet<>();
  ps.forEach(x -> {
   if (x.getTopics() != null && !x.getTopics().isBlank()) {
    topics.addAll(Arrays.asList(x.getTopics().split(", ")));
   }
  });
  List<ResearchGap> out = new ArrayList<>();
  for (String topic : topics) {
   String statement = "Limited evidence for " + topic + " under the stated constraints";
   out.add(gaps.save(new ResearchGap(p, statement, DEFAULT_GAP_CONFIDENCE,
